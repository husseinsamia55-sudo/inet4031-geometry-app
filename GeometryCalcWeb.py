...
def sphereForm():
   if request.method == "POST":
       # get the radius from the form (same name as in the HTML)
       radius_str = request.form.get("rad", "")
       if radius_str:
           r = float(radius_str)
           # call sphere.volume using only radius
           vol = sphere.volume(r)
           # render the template and show the result
           return render_template("sphere.html", radius=r, volume=vol)
       else:
           # no radius entered, just redisplay form with no result
           return render_template("sphere.html", radius=None, volume=None)
